...
namespace Alexandrie.v2.Tests
{
  ...
  public class TestsRequêtes
  {
    ...
    public void RechercheAdhérentsParVille ()
    {
      ...
      Assert.That ((résultat.First () as Requêtes.RésuméAdherent).Numéro, Is.EqualTo (23));
    ...
    public void RechercheAdhérentsParNom ()
    {
      ...
      Assert.That (résultat.Count (), Is.EqualTo (1));
      Assert.That ((résultat.First () as Requêtes.RésuméAdherent).Numéro, Is.EqualTo (48));
    ...
    public void RechercheEmpruntsActifsParDescription ()
    {
      ...
      Assert.That (résultat.Count (), Is.EqualTo (1));
      Assert.That ((résultat.First () as Requêtes.RésuméEmprunt).Identifiant, Is.EqualTo (1111));
      Assert.That (résultat.First ().Description, Is.EqualTo ("Les 3 mousquetaires"));
      Assert.That (résultat.First ().Emprunteur.Nom, Is.EqualTo ("Martin"));
    }
    ...
    public void RechercheEmpruntsActifsParNomAdherent ()
    {
      ...
      Assert.That (résultat.Count (), Is.EqualTo (1));
      Assert.That ((résultat.First () as Requêtes.RésuméEmprunt).Identifiant, Is.EqualTo (3333));
      Assert.That (résultat.First ().Description, Is.EqualTo ("Le Retour de Martin Guerre"));
      Assert.That (résultat.First ().Emprunteur.Nom, Is.EqualTo ("Dupont"));
    }
    ...
    public void RechercheEmpruntsActifsParDescriptionOuNomAdherent ()
    {
      ...
      Assert.That (résultat.Select (r => (r as Requêtes.RésuméEmprunt).Identifiant), Is.EquivalentTo (new long[] {
        1111,
        3333
      }));
    }
